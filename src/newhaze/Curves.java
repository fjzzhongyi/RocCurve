...
public class Curves {
	...
	private static void init() {
		File file_gasdm = new File("C://Users//Shuai//Documents//GitHub//RocCurve//data//haze//GASDM-result-haze.txt");
		File file_nphgs = new File("C://Users//Shuai//Documents//GitHub//RocCurve//data//haze//NPHGS-result-haze.txt");		
		File file_eventtree = new File("C://Users//Shuai//Documents//GitHub//RocCurve//data//haze//EventTree-result-haze.txt");
		File file_lgta = new File("C://Users//Shuai//Documents//GitHub//RocCurve//data//haze//LGTA-result-haze.txt");
		File file_fss = new File("C://Users//Shuai//Documents//GitHub//RocCurve//data//haze//FSS-result-haze.txt");
		File file_real = new File("C://Users//Shuai//Documents//GitHub//RocCurve//data//haze//TrueLable-haze.txt");
		...
		BufferedReader reader_fss = null;
		...
		try {
			...
			reader_fss = new BufferedReader(new FileReader(file_fss));
			...
			while ((str_gasdm = reader_gasdm.readLine()) != null && (str_nphgs = reader_nphgs.readLine()) != null
					...
					&& (str_fss = reader_fss.readLine()) != null && (str_real = reader_real.readLine()) != null) {
				String[] str_gasdms;
				String[] str_nphgss;
				...
				for (int i=0; i<34; i++) {
					...
					fss[fssi][i] = Integer.parseInt(str_fsss[i+2]);
	...
	private static ArrayList<double[]> foreDete(String type) {
		...
		for (int i=T; i<gasdmi-T; i++) {
			...
			if (type.equals("gasdm")){
				...
				temp = compare(fss[i], win);//TP,TN,FP,FN
			}
	...
	private static void output(String title, String xlabel, String ylabel, ArrayList<Point> gasdm,
		...
		try {
			...
			for (int i=0; i<fss.size(); i++) {
				writer.write(Double.toString(fss.get(i).x));
				writer.write(" ");
				writer.write(Double.toString(fss.get(i).y));
				writer.newLine();
			}
			writer.close();
		} catch (IOException e) {
	...
	private static void plotForeDete() {
		...
		ArrayList<Point> plotfss = new ArrayList<Point>();
		...
		plotfss = TPRCurve(resultfss);
		DrawPlot.draw("FPR vs TPR(Forecasting and detection)", "False Positive Rate(From 0-1 FP Per-day)", 
				"True Positive Rate(Forecasting and detection)", plotgasdm, plotnphgs, ploteventtree, plotlgta, plotfss);
		...
		plotfss = leadCurve(resultfss);
		DrawPlot.draw("FPR vs Lead Time(Forecasting)", "False Positive Rate(From 0-1 FP Per-day)", 
				"Lead Time(Forecasting)", plotgasdm, plotnphgs, ploteventtree, plotlgta, plotfss);
		...
		plotfss = lagCurve(resultfss);
		DrawPlot.draw("FPR vs Lag Time(Detection)", "False Positive Rate(From 0-1 FP Per-day)", 
				"Lag Time(Detection)", plotgasdm, plotnphgs, ploteventtree, plotlgta, plotfss);
	...
	private static void plotFore() {
		...
		DrawPlot.draw("FPR vs TPR(Forecasting)", "False Positive Rate(From 0-1 FP Per-day)", 
				"True Positive Rate(Forecasting)", plotgasdm, plotnphgs, ploteventtree, plotlgta, plotfss);
