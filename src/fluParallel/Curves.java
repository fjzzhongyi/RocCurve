...
public class Curves {
	...
	private static void init() {
		File file_gasdm = new File("data//flu//GASDM_flu.txt");
		File file_nphgs = new File("data//flu//NPHGS_flu-alpha=1.0.txt");
		File file_eventtree = new File("data//flu//EventTree_flu.txt");
		File file_pasd = new File("data//flu//PASD_flu.txt");
		File file_real = new File("data//flu//trueflu_K=1000.txt");
		BufferedReader reader_gasdm = null;
		BufferedReader reader_nphgs = null;
		BufferedReader reader_eventtree = null;
		BufferedReader reader_pasd = null;
		BufferedReader reader_real = null;
		gasdm = new int[80][10];
		nphgs = new int[80][10];
		eventtree = new int[80][10];
		pasd = new int[80][10];
		real = new int[80][10];
		try {
			...
			reader_pasd = new BufferedReader(new FileReader(file_pasd));
			...
			String str_pasd = null;
			String str_real = null;
			while ((str_gasdm = reader_gasdm.readLine()) != null && (str_nphgs = reader_nphgs.readLine()) != null
					&& (str_eventtree = reader_eventtree.readLine()) != null && (str_pasd = reader_pasd.readLine()) != null
				...
				for (int i=0; i<10; i++) {
					...
					pasd[pasdi][i] = Integer.parseInt(str_pasds[i+2]);
	...
	private static ArrayList<double[]> foreDete(String type) {
		...
		for (int i=T; i<gasdmi-T; i++) {
			...
			} else if (type.equals("pasd")) {
				temp = compare(pasd[i], win);//TP,TN,FP,FN
			}
	...
	private static void output(String title, String xlabel, String ylabel, ArrayList<Point> gasdm,
			ArrayList<Point>nphgs, ArrayList<Point>eventtree, ArrayList<Point>pasd){
		...
		try {
			...
			writer.write("PASD: ");
			writer.newLine();
			for (int i=0; i<pasd.size(); i++) {
				writer.write(Double.toString(pasd.get(i).x));
				writer.write(" ");
				writer.write(Double.toString(pasd.get(i).y));
				writer.newLine();
			}
			writer.close();
		} catch (IOException e) {
	...
	private static void plotForeDete() {
		...
		plotpasd = leadCurve(resultpasd);
		DrawPlot.draw("FPR vs Lead Time(Forecasting)", "False Positive Rate(From 0-1 FP Per-day)", 
				"Lead Time(Forecasting)", plotgasdm, plotnphgs, ploteventtree, plotpasd);
		...
		plotpasd = lagCurve(resultpasd);
		DrawPlot.draw("FPR vs Lag Time(Detection)", "False Positive Rate(From 0-1 FP Per-day)", 
				"Lag Time(Detection)", plotgasdm, plotnphgs, ploteventtree, plotpasd);
	...
	private static void plotFore() {
		...
		DrawPlot.draw("FPR vs TPR(Forecasting)", "False Positive Rate(From 0-1 FP Per-day)", 
				"True Positive Rate(Forecasting)", plotgasdm, plotnphgs, ploteventtree, plotpasd);
